...
import PatientFilters from './PatientFilters.vue'
import PatientOptions from './PatientOptions.vue'
const patients = ref([])
...
const patient_url = ref(`patients?is_active=true`)
...
const getPatientStats = () => {
  ApiClient()
    .get('patient/stats')
    .then((res) => {
      emitter.emit('loadPatientStats', res.data)
    })
    .catch((error) => {
      console.log(error)
    })
...
getPatients(patient_url.value)
getPatientStats()
emitter.on('refreshPatients', () => {
  getPatients(patient_url.value)
  getPatientStats()
})
...
emitter.on('setPatientFilters', (data) => {
  filters.value = data.selectedFilters
  patient_url.value = `patients?is_active=${filters.value.is_active}&&blood_group=${filters.value.blood_group}&&hiv_status=${filters.value.hiv_status}&&gender=${filters.value.gender}`
  getPatients(patient_url.value)
  addFilters()
})
</script>
<template>
  <div>
    ...
    >
      ...
      <p class="text-main-100 text-sm mt-1">Please add some patients to view them here.</p>
      <button
        @click="openNewPatient"
    ...
    <div v-if="!isLoading && patients.length" class="mt-5 overflow-x-auto rounded-lg">
          ...
          >
            ...
            <th scope="row" class="px-6 py-4">
              <div class="flex gap-2 items-center">
                ...
                <span
                  v-else
                  class="bg-emerald-500/80 text-white flex items-center justify-center font-normal text-xs uppercase size-8 rounded-full"
                >
            ...
            <td class="px-6 py-4">
              <div class="">
                ...
                <p class="text-blue-40">{{ patient.email }}</p>
              </div>
            </td>
            <td class="px-6 py-4">
              {{ patient.age }}
            ...
            <td class="px-6 py-4">{{ formatDate(patient.created_at) }}</td>
            <td class="px-6 py-4">{{ formatDate(patient.last_visit) }}</td>
            <td class="px-6 py-4">
