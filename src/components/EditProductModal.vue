...
const editProduct = () => {
  ...
  data.append('category', product.value.category.id)
  data.append('vendor', product.value.vendor.id)
  ...
  ApiClient('multipart/formdata')
    .put(`products/${product.value.id}/`, data)
    .then(() => {
...
emitter.on('editProduct', (data) => {
  product.value = data.product
  product.value.category = product.value.category.id
  product.value.vendor = product.value.vendor.id
  openModal()
})
            ...
            >
              ...
              <div class="mt-5 flex justify-between gap-5 text-sm">
                <button
                  v-if="!deleting"
                  @click="deleteProduct"
                  class="bg-red-500 inline-flex gap-2 items-center justify-center text-white w-full py-2 rounded-md"
                >
                  Delete
                </button>
                <button
