...
let currentForm = ref('email')

const sendCode = () => {
  requestingNewCode.value = true
  AuthClient()
    .post('password/reset', { email: user.value.email, action: 'reset' })
    .then((res) => {
      if (res.data.success) {
...
const verifyCode = () => {
  ...
  AuthClient()
    .post('code/verify', { code: user.value.code, email: user.value.email })
    .then((res) => {
      if (res.data.success) {
...
const setNewPassword = () => {
  ...
  AuthClient()
    .post('password/new', { password: user.value.password, email: user.value.email })
    .then((res) => {
      if (res.data.success) {
    ...
    <div class="flex h-full">
      <div class="w-1/2 h-full bg-white">
        <div class="p-10 flex flex-col items-center h-full justify-center">
          ...
          <div v-if="currentForm == 'code'" class="w-full mt-5 max-w-md">
            <p class="text-3xl font-bold">Check your inbox</p>
            <p class="mt-3 text-main-100 text-sm">
              We have sent reset instructions to <span class="text-main">{{ user.email }}</span
            ...
            <div class="flex mt-5 flex-col gap-2 text-sm">
              ...
              />
              <small v-if="codeError" class="text-xs text-red-500">Invalid code.</small>
            </div>
            ...
            <div class="mt-5 flex gap-2 text-sm">
              ...
              <span v-else @click="sendCode" class="text-blue-50 cursor-pointer">Resend</span>
            </div>
          </div>

          <div v-if="currentForm == 'password'" class="w-full mt-5 max-w-md">
            <p class="text-3xl font-bold">Set your password</p>
            <p class="mt-3 text-main-100 text-sm">Use the form below to set up a new password.</p>
            <div class="flex mt-5 flex-col gap-2 text-sm">
              ...
              <small v-if="passwordError" class="text-xs text-red-500"
                >Password must be 8 characters alphanumeric.</small
              >
            </div>
            <div class="flex mt-5 flex-col gap-2 text-sm">
              ...
              <input
                ...
                class="bg-light-200 text-main-200 py-2 rounded-md border-0 focus:ring-0 focus:outline-none"
              />
              <!-- <small v-if="passwordError" class="text-xs text-red-500">Password must be 8 characters alphanumeric.</small> -->
            </div>

            <div class="mt-10">
              <button
                v-if="!settingNewPassword"
            ...
            <div class="mt-5 flex gap-2 text-sm">
              ...
              <a href="/login" class="text-blue-50">Login</a>
            </div>
          </div>

          <div v-if="currentForm == 'email'" class="w-full mt-5 max-w-md">
            <p class="text-3xl font-bold">Forgot your password</p>
            <p class="mt-3 text-main-100 text-sm">
            ...
            <div class="flex mt-5 flex-col gap-2 text-sm">
              ...
              <input
                ...
                class="bg-light-200 text-main-200 py-2 rounded-md border-0 focus:ring-0 focus:outline-none"
              />
              <small v-if="emailError" class="text-xs text-red-500">Email not found.</small>
            </div>

            <div class="mt-10">
              <button
                v-if="!requestingNewCode"
                @click="sendCode"
                :disabled="user.email == ''"
                class="bg-blue-50 disabled:opacity-50 text-white py-2 rounded-md w-full text-sm"
              >
